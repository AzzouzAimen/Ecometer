...
const createObjectif = async (req, res) => {
  const { clientId, year, objectif, objectifType, objectifDescription } = req.body;
...
const updateObjectif = async (req, res) => {
  const { clientId, objectifId, objectif, objectifType, objectifDescription } = req.body;
  if (!isValidObjectId(clientId)) {
    return res.status(400).json({ msg: "Invalid client ID" });
  }
  try {
...
module.exports = { createObjectif, updateObjectif, deleteObjectif, getAllObjectifs };
